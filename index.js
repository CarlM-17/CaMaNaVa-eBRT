...
app.get('/api/category', async (req, res) => {
  try {
    ...
    const byArea = Object.values(areaMap).sort((a, b) => b.sales - a.sales);
    ...
    res.json({ success: true, summary, detail, byArea, movers });
  } catch (err) {
...
<main class="main">
  ...
  <div class="tab-content" id="tab-category">
    ...
    <div class="charts-grid">
      <div class="chart-card">
        <div class="chart-title"><i class="fa fa-chart-bar"></i> Sales by Category — Current vs Last Year</div>
        <div class="chart-legend">
          <span class="legend-item"><span class="legend-dot" style="background:#6366f1;color:#6366f1"></span>Sales Current</span>
          <span class="legend-item"><span class="legend-dot" style="background:#3a4560;color:transparent;box-shadow:none"></span>Sales LY</span>
        </div>
      ...
      <div class="chart-card">
        <div class="chart-title"><i class="fa fa-chart-pie"></i> Category Mix</div>
    ...
    <!-- Charts Row 2: Top Movers & Area Distribution -->
    <div class="charts-grid">
      <div class="chart-card">
        <div class="chart-title">
      ...
      <div class="chart-card">
        <div class="chart-title"><i class="fa fa-layer-group"></i> Sales by Area</div>
        <div style="position:relative;height:380px">
          <canvas id="cAreaChart"></canvas>
        </div>
      </div>
    </div>
...
function renderCCharts(summary, byArea, movers) {
  // ─── Category bar chart (Current vs LY) ─────────────────────────────────
  if (cCategoryChartInst) cCategoryChartInst.destroy();
  const cat = document.getElementById('cCategoryChart');
  const catCtx = cat.getContext('2d');
  const top = summary.slice(0, IS_MOBILE ? 8 : 14);
  const gradients = top.map((r,i) => {
    const c = colorForCategory(r.category, i);
    const g = catCtx.createLinearGradient(0, 0, 0, 320);
    g.addColorStop(0, c + 'ff'); g.addColorStop(1, c + '99');
    return g;
  });
  cCategoryChartInst = new Chart(cat, {
    ...
    data: {
      ...
      datasets: [
        { label: 'Sales', data: top.map(r=>r.sales), backgroundColor: gradients, borderRadius: 6, borderSkipped: false, barPercentage: 0.7 },
        { label: 'Sales LY', data: top.map(r=>r.salesLY), backgroundColor: 'rgba(74,83,107,0.4)', borderColor: 'rgba(148,163,200,0.2)', borderWidth: 1, borderRadius: 6, borderSkipped: false, barPercentage: 0.7 }
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: {
        legend: { display: false },
        tooltip: { backgroundColor:'rgba(15,20,35,0.95)', borderColor:'rgba(99,102,241,0.4)', borderWidth:1, padding:12, cornerRadius:10, titleFont:{family:"'Space Grotesk'",size:13,weight:'600'}, bodyFont:{family:"'JetBrains Mono'",size:11.5}, titleColor:'#f0f3fb', bodyColor:'#a8b3d1', callbacks: { label: ctx => \` \${ctx.dataset.label}: ₱\${ctx.raw.toLocaleString('en-PH',{minimumFractionDigits:2})}\` } }
      },
      scales: {
        x: { grid:{color:'rgba(148,163,200,0.04)',drawBorder:false}, ticks:{color:'#a8b3d1',font:{size:10,family:"'Inter'",weight:'500'},maxRotation:40} },
        y: { grid:{color:'rgba(148,163,200,0.06)'}, ticks:{color:'#a8b3d1',font:{size:10.5,family:"'JetBrains Mono'",weight:'500'}, callback: v => v >= 1e6 ? '₱'+(v/1e6).toFixed(1)+'M' : v >= 1e3 ? '₱'+(v/1e3).toFixed(0)+'K' : v } }
  ...
  cCategoryPieInst = new Chart(pie, {
    ...
    options: {
      ...
      plugins: {
        legend: { position:'bottom', labels: { color:'#a8b3d1', font:{size:10.5,family:"'Inter'",weight:'500'}, padding:12, boxWidth:10, boxHeight:10, generateLabels: chart => chart.data.labels.map((label,i)=>({ text: \`\${label}  \${fmt(summary[i].sales)}\`, fillStyle: pieColors[i], strokeStyle:'transparent', lineWidth:0, pointStyle:'rectRounded' })) } },
        tooltip: { backgroundColor:'rgba(15,20,35,0.95)', borderColor:'rgba(99,102,241,0.4)', borderWidth:1, padding:12, cornerRadius:10, titleFont:{family:"'Space Grotesk'",weight:'600',size:13}, bodyFont:{family:"'JetBrains Mono'",size:11.5}, titleColor:'#f0f3fb', bodyColor:'#a8b3d1',
          callbacks: { label: ctx => { const total = ctx.dataset.data.reduce((a,b)=>a+b,0); const pct = total ? ((ctx.raw/total)*100).toFixed(1) : 0; return \` ₱\${ctx.raw.toLocaleString('en-PH',{minimumFractionDigits:2})}  (\${pct}%)\`; } }
        }
      }
    }
  });

  // ─── Movers Chart (Top 8 + Bottom 8 sub-departments by diffVal) ────────
  ...
  if (combined.length) {
    cMoversChartInst = new Chart(movCanvas, {
      ...
      options: {
        ...
        plugins: {
          legend: { display:false },
          tooltip: { backgroundColor:'rgba(15,20,35,0.95)', borderColor:'rgba(99,102,241,0.4)', borderWidth:1, padding:12, cornerRadius:10, titleFont:{family:"'Space Grotesk'",size:13,weight:'600'}, bodyFont:{family:"'JetBrains Mono'",size:11.5}, titleColor:'#f0f3fb', bodyColor:'#a8b3d1',
            callbacks: { label: ctx => { const m = combined[ctx.dataIndex]; return [' Diff: ₱'+ctx.raw.toLocaleString('en-PH',{minimumFractionDigits:2}), ' Sales: ₱'+m.sales.toLocaleString('en-PH',{minimumFractionDigits:2}), ' Sales LY: ₱'+m.salesLY.toLocaleString('en-PH',{minimumFractionDigits:2}), ' Category: '+m.category]; }, title: ctx => combined[ctx[0].dataIndex].subDepName }
          }
        },
        scales: {
          ...
          y: { grid:{color:'rgba(148,163,200,0.04)',drawBorder:false}, ticks:{color:'#a8b3d1',font:{size:10.5,family:"'Inter'",weight:'500'}, autoSkip: false } }
        }
      }
    });
  }
  ...
  // ─── Area Distribution ──────────────────────────────────────────────────
  if (cAreaChartInst) cAreaChartInst.destroy();
  const areaCanvas = document.getElementById('cAreaChart');
  const areaCtx = areaCanvas.getContext('2d');
  const areaColors = byArea.map(a => AREA_COLORS[a.area] || DEFAULT_COLOR);
  const areaGrads = byArea.map(a => {
    const [c1,c2] = AREA_GRADIENTS[a.area] || [DEFAULT_COLOR, DEFAULT_COLOR];
    const g = areaCtx.createLinearGradient(0,0,400,0);
    g.addColorStop(0, c1); g.addColorStop(1, c2);
    return g;
  });
  cAreaChartInst = new Chart(areaCanvas, {
    type: 'bar',
    data: {
      labels: byArea.map(a => a.area),
      datasets: [
        { label: 'Sales', data: byArea.map(a=>a.sales), backgroundColor: areaGrads, borderRadius: 8, borderSkipped: false, barPercentage: 0.55 },
        { label: 'Sales LY', data: byArea.map(a=>a.salesLY), backgroundColor: 'rgba(74,83,107,0.5)', borderColor: 'rgba(148,163,200,0.2)', borderWidth: 1, borderRadius: 8, borderSkipped: false, barPercentage: 0.55 }
      ]
    },
    options: {
      indexAxis: 'y', responsive: true, maintainAspectRatio: false,
      plugins: {
        ...
        tooltip: { backgroundColor:'rgba(15,20,35,0.95)', borderColor:'rgba(99,102,241,0.4)', borderWidth:1, padding:12, cornerRadius:10, titleFont:{family:"'Space Grotesk'",size:13,weight:'600'}, bodyFont:{family:"'JetBrains Mono'",size:11.5}, titleColor:'#f0f3fb', bodyColor:'#a8b3d1',
          callbacks: { label: ctx => { const a = byArea[ctx.dataIndex]; const diff = a.sales - a.salesLY; const pct = a.salesLY ? (diff/a.salesLY*100).toFixed(2) : 0; return \` \${ctx.dataset.label}: ₱\${ctx.raw.toLocaleString('en-PH',{minimumFractionDigits:2})}\` + (ctx.datasetIndex === 0 ? \`  (\${diff>=0?'+':''}\${pct}%)\` : ''); } }
        }
      },
      scales: {
        x: { grid:{color:'rgba(148,163,200,0.06)'}, ticks:{color:'#a8b3d1',font:{size:10.5,family:"'JetBrains Mono'",weight:'500'}, callback: v => v >= 1e6 ? '₱'+(v/1e6).toFixed(1)+'M' : v >= 1e3 ? '₱'+(v/1e3).toFixed(0)+'K' : v } },
        y: { grid:{color:'rgba(148,163,200,0.04)',drawBorder:false}, ticks:{color:'#a8b3d1',font:{size:11,family:"'Inter'",weight:'600'}} }
      }
    }
  });
}
